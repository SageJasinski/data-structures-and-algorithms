...
const { val, prop } = require("cheerio/lib/api/attributes");
...
const screenForNames = (arr) => {
  let newArr = [];
  let dump = [];
  let reg = /^([Mr]?[Dr]?[Mrs])+(.\s[A-Za-z])/g;

  arr.map(value => reg.test(value) === true ? newArr.push(value):dump.push(value));
  // console.log(newArr);
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  ...
  arr.filter(value => {
    ...
    if(parseInt(value.mass) > lukeMass){
...
const sortBy = (property, arr) => {
  // let check =  arr.sort((a,b) => { a.`${property}` - b${property}});
  // console.log(check);
};
