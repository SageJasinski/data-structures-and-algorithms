...
const firstLetters = (arr) => {
  // newaRR= [];
  // let reg = /[a-zA-Z]/;
  let firstWord = arr.map(val => val.split("")[0]);
  return firstWord;
};
...
const findHappiness = (arr) => {
  // let reg = /\:\)/;
  let newArr = [];
  arr.map(val => { if(val.includes(':)') === true){newArr.push(val);}});
  return newArr;
};
...
const onlyOddChars = (str) => {
  let even =[];
  let odd =[];
  let mpa = function(){ for (let i = 0; i < str.length; i++){
    i % 2 === 0 ? even.push(str[i]):odd.push([i]);
  }};
  let test = even.join('');
  return even;
};
